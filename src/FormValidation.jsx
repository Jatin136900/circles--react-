...
const App = () => {
      ...
      if (data[obj.element] !== null) {
        ...
        if (obj.rule.required) {
          if (formElem.length === 0) {
            newErrors[obj.element] = "This field is required";
            return
          }
        }
        ...
        if (obj.rule.minLength) {
          if (formElem.length < obj.rule.minLength)
            newErrors[obj.element] = `The minimum input should be ${obj.rule.minLength} characters`;
          return
        }

        if (obj.rule.maxLength) {
          if (formElem.length > obj.rule.maxLength)
            newErrors[obj.element] = `The maximum input should be ${obj.rule.maxLength} characters`;
          return
        }

        if (obj.rule.regex) {
          if (!obj.rule.regex.test(formElem))
            newErrors[obj.element] = "please specify value as per the required format";
          return
        }
      }
    });
  ...
  return (
    <>
      <div className="form-wrapper font-[neue-machina] min-h-screen flex items-center justify-center bg-gray-100">
